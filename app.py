...
st.markdown("##### 🐾 資安警犬隊出動！結合被動 AI 鑑識、熱圖定位與【主動式防 AI 變造疫苗】技術")
...
# 3. 殺手級模組：主動式防 AI 變造疫苗 (Adversarial Noise Protection)
...
# 4. 殺手級模組：Copilot 白話鑑定解說員 (Explainable AI)
def generate_copilot_narrative(ai_score, trust_score, roi_box, prnu_has, is_smoothed, c2pa_list):
    narrative = []
    narrative.append(f"🐕 **警犬 Copilot 綜合簡報**：本張影像獲得資安可信度 **{trust_score:.1f} / 100 分**。")
    
    if ai_score > 60:
        narrative.append(f"⚠️ **AI 深度偽造警訊**：深度學習網路在像素特徵層捕捉到高度擴散模型 (Diffusion) 生成殘影，偽造機率高達 {ai_score:.1f}%。")
    else:
        narrative.append("✅ **AI 辨識正常**：影像整體特徵符合真實光學成像規律。")
        
    if roi_box:
        narrative.append(f"🔍 **壓縮差異異常**：在座標 (X:{roi_box[0]}, Y:{roi_box[1]}) 區域發現 ELA 熱圖高亮異常，疑有局部 P 圖或抹除合成痕跡。")
        
    if not prnu_has:
        narrative.append("⚠️ **硬體指紋缺失**：提取不到感光晶片 (CMOS) 物理雜訊，研判此圖非真實相機直出，曾經數位繪圖或繪圖軟體匯出。")
        
    if is_smoothed:
        narrative.append("⚠️ **反偵查邊緣抹除**：高頻雜訊變異度過低，疑有使用磨皮或高斯模糊遮掩修圖邊緣之行為。")
...
# 分頁選單：鑑識模式 vs 主動防禦模式
tab1, tab2 = st.tabs(["🔍 被動真偽鑑識大腦", "🛡️ 主動式【防 AI 盜圖疫苗】注入"])
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        copilot_text = generate_copilot_narrative(ai_score, trust_score, roi_box, has_prnu, is_smoothed, c2pa_findings)
...
with tab2:
    ...
    st.markdown("在將照片發布至社群平台 (IG / FB) 前，透過本功能注入人眼無感之【低頻對抗性雜訊】。未來若有歹徒或 AI 模型欲對保護後之照片進行二次改圖或 Deepfake 訓練，**將引發 AI 神經網路計算崩潰**！")
    
    protect_file = st.file_uploader("📂 上傳您欲保護之原創照片", type=["jpg", "jpeg", "png"], key="protect_upload")
    if protect_file is not None:
        p_img = Image.open(protect_file)
        v_intensity = st.slider("選擇疫苗保護強度 (建議選擇 3% ~ 5%)", 0.01, 0.10, 0.03)
        
        if st.button("💉 立即注入「防 AI 變造疫苗」"):
            with st.spinner("正在對影像注入對抗性微小雜訊..."):
                protected_img = apply_ai_vaccine(p_img, intensity=v_intensity)
                
                col_p1, col_p2 = st.columns(2)
                with col_p1:
                    st.image(p_img, caption="原始照片", use_container_width=True)
                with col_p2:
                    st.image(protected_img, caption="✅ 已注入防 AI 疫苗之保護照片 (視覺幾乎無差異)", use_container_width=True)
                ...
                st.success("🎉 保護完成！此照片現已具備抵抗 AI 二次變造之免疫力。")
