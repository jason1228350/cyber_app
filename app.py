...
# 頁面配置
st.set_page_config(
    page_title="影真鑑 - 調查局級數位證物鑑識平台",
    page_icon="🛡️",
    layout="wide"
)

st.title("🛡️ 影真鑑：法醫級數位證物與影像鑑識平台")
st.markdown("國家級數位鑑識標準：結合 AI 特徵、頻域 FFT、PRNU 感光指紋、C2PA 水印與 SHA-256 證物指紋")
st.success("✅ 全防線資安與刑事採證鑑識引擎已成功啟動！")
...
def generate_ela_jet(image, quality=90):
    ...
    ela_im = ImageEnhance.Brightness(ela_im).enhance(scale * 2.0)
    ...
    jet_array = (np.stack([r, g, b], axis=-1) * 255).astype(np.uint8)
    
    return Image.fromarray(jet_array), gray_array

# 自動 ROI 變造疑點標記 (Bounding Box)
def detect_roi_bounding_box(image_orig, gray_ela_array):
    h, w = gray_ela_array.shape
    threshold = np.percentile(gray_ela_array, 98)  # 抓取最高 2% 異常數值
...
def extract_prnu_noise(image):
    ...
    has_sensor_prnu = noise_var > 15.0  # 實體相機具有較高感光雜訊
...
# 上傳 UI
uploaded_file = st.file_uploader("📂 請選擇待鑑識之數位證物照片", type=["jpg", "jpeg", "png", "webp"])
...
if uploaded_file is not None:
    ...
    st.image(image, caption="原始證物照片", use_container_width=True)
    ...
    if classifier:
        ...
        ai_score = fake_score
        ai_result_text = "⚠️ 高度懷疑為 AI 生成假圖" if ai_score > 60 else "✅ 未見明顯 AI 偽造痕跡"

    # 計算綜合資安可信度評分 (Trust Score)
    ...
    # ------------------ 綜合資安可信度評分卡 ------------------
    st.markdown("---")
    st.subheader("📊 證物綜合資安可信度評分卡 (Trust Score)")
    col_s1, col_s2 = st.columns([1, 2])
    with col_s1:
        st.metric(label="資安可信度總分", value=f"{trust_score:.1f} / 100")
    with col_s2:
        if trust_score >= 80:
            st.success("🟢 **法庭證物評等：高可信度** (未見重大變造或 AI 偽造痕跡)")
        elif trust_score >= 50:
            st.warning("🟡 **法庭證物評等：中度風險** (發現二次編輯、磨皮或採證缺失)")
        else:
            st.error("🔴 **法庭證物評等：高度偽造證物** (高度懷疑 AI 生成或重度修圖變造)")

    # ------------------ 第一防線：AI 檢測 ------------------
    st.markdown("---")
    st.subheader("🤖 第一防線：AI 深度學習偽造特徵辨識")
    st.write(f"**AI 偽造風險指數：{ai_score:.2f}%** - {ai_result_text}")

    # ------------------ 第二防線：ELA 彩色熱圖與 ROI 標記 ------------------
    st.markdown("---")
    st.subheader("🔥 第二防線：ELA 彩色熱力圖與自動 ROI 變造區域定位")
    col_e1, col_e2 = st.columns(2)
    with col_e1:
        st.image(ela_jet_img, caption="ELA 彩色熱感應圖 (紅色代表極高異常壓縮區)", use_container_width=True)
    with col_e2:
    ...
    # ------------------ 第三防線：FFT 頻域與 PRNU 感光指紋 ------------------
    st.markdown("---")
    st.subheader("📉 第三防線：FFT 頻域殘影與 PRNU 物理感光指紋")
    col_f1, col_f2 = st.columns(2)
    with col_f1:
        st.image(fft_img, caption="FFT 快速傅立葉變換頻域圖 (中央網格放射代表 AI 演算法殘影)", use_container_width=True)
    with col_f2:
    ...
    # ------------------ 第四防線：反偵查、LSB 與 C2PA 水印 ------------------
    st.markdown("---")
    st.subheader("🕵️ 第四防線：反偵查、LSB 隱寫術與 C2PA 標籤稽核")
    col_a1, col_a2 = st.columns(2)
    with col_a1:
        st.image(lsb_img, caption="LSB 最低有效位點陣圖", use_container_width=True)
    with col_a2:
        st.markdown(f"**高頻平滑度指標:** `{lap_var:.2f}`")
        if is_smoothed:
    ...
    # ------------------ 第六防線：EXIF 中文化 ------------------
    st.markdown("---")
    st.subheader("📜 第六防線：EXIF 數位履歷元資料稽核")
    exif_data_dict = {}
    info = image._getexif()
    if info:
        ...
        exif_data_dict = {"狀態說明": "未偵測到原始相機 EXIF 元資料 (可能為截圖或網路轉傳)"}
        st.warning("⚠️ 未偵測到原始相機 EXIF 拍攝紀錄。")

    # ------------------ 第七防線：報告匯出 ------------------
    st.markdown("---")
    st.subheader("📄 第七防線：法庭級資安採證稽核報告匯出")
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    report_content = f"""================================================================================
           影真鑑 - 法務部與調查局級數位證物採證稽核報告 (Official Report)
================================================================================
...
- AI 偽造指數: {ai_score:.2f}% ({ai_result_text})
- 疑點 ROI 座標: {roi_box if roi_box else "未發現集中疑點區塊"}

【三、高階物理解析】
- PRNU 感光指紋: {"✅ 具備實體鏡頭晶片雜訊" if has_prnu else "⚠️ 無實體鏡頭雜訊 (疑為AI/數位渲染)"}
...
================================================================================
此報告由「調查局級數位證物與影像鑑識平台」自動生成並完成 SHA-256 數位簽章鎖定
================================================================================
"""

    st.download_button(
        label="📥 一鍵下載法庭級數位採證報告 (.txt)",
        data=report_content,
        file_name=f"法庭採證報告_{sha256_hash[:10]}.txt",
        mime="text/plain"
    )
